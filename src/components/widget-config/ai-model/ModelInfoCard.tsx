...
export function ModelInfoCard({ selectedModelId }: ModelInfoCardProps) {
  ...
  const getModelCapabilityScore = () => {
    if (!selectedModel) return 50;
    
    // This is a simple heuristic - in a real app you'd have real metrics
    const modelTypeFactor = selectedModel.type.toLowerCase().includes('large') ? 80 : 60;
    const configFactor = selectedModel.configuration?.temperature ? 
      Math.max(40, 100 - selectedModel.configuration.temperature * 50) : 70;
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white rounded-md p-2 border border-blue-100">
                    ...
                    <div className="space-y-1.5">
                      <Progress value={getModelCapabilityScore()} className="h-1.5" />
                      <p className="text-[10px] text-gray-500">
                        {getModelCapabilityScore() >= 75 ? "High" : 
                          getModelCapabilityScore() >= 50 ? "Medium" : "Basic"} capability model
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-md p-2 border border-blue-100">
                    ...
                    <div className="flex items-baseline">
                      <span className="text-sm font-medium">
                        {selectedModel.usageCount?.toLocaleString() || 0}
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      {selectedModel.configuration.temperature !== undefined && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{selectedModel.configuration.temperature}</span>
                        </div>
                      )}
                      {selectedModel.configuration.maxTokens !== undefined && (
                        <div className="flex justify-between">
                          <span className="text-gray-500">Max Tokens:</span>
                          <span className="font-medium">{selectedModel.configuration.maxTokens}</span>
                        </div>
                      )}
                      {selectedModel.configuration.systemPrompt && (
